...
const logPool=[
  ...
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'P → Q가 참일 때, 반드시 참인 명제는?',opts:['Q → P','¬P → ¬Q','¬Q → ¬P','P ↔ Q'],correct:2,difficulty:4},
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'5명이 원형으로 앉을 때, 경우의 수는?',opts:['24','48','60','120'],correct:0,difficulty:4},
  ...
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'어떤 집합의 모든 부분집합의 수가 64개라면, 원소의 수는?',opts:['4개','5개','6개','7개'],correct:2,difficulty:4},
  ...
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'집합 A에 n개, B에 m개의 원소가 있을 때, A∪B의 최대 원소 수는?',opts:['n+m-1','n+m','n×m','max(n,m)'],correct:1,difficulty:3},
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'P↔Q가 참이고 Q→R이 참이면, P와 R의 관계는?',opts:['P→R은 반드시 참','R→P는 반드시 참','P↔R이 반드시 참','관계없음'],correct:0,difficulty:5},
  {type:'logic',typeLabel:'논리 추론',q:'다음 논리 문제를 풀어보세요.',premise:'배중률(Law of Excluded Middle)이란?',opts:['A이면서 ¬A일 수 없다','A이거나 ¬A 중 하나는 반드시 참이다','A이면 B이다','A는 A이다'],correct:1,difficulty:5}
];
...
const extendedTests={
  ...
  speed:{id:'speed',title:'정보 처리 속도',icon:'⚡',color:'#059669',mean:77,sd:8,
    questions:[
      {q:'다음 중 소수(Prime Number)가 아닌 것은?',opts:['17','19','21','23'],correct:2},
      ...
      {q:'64의 제곱근은?',opts:['6','7','8','9'],correct:2},
  ...
  creativity:{id:'creativity',title:'창의성 지수',icon:'🎨',color:'#db2777',mean:73,sd:9,
    questions:[
      {q:'가장 창의적인 문제 해결 접근은?',opts:['기존 방법 반복','다른 분야 방법 차용','전문가에게 의뢰','더 열심히 같은 방법 시도'],correct:1},
      {q:'창의적 문제 해결의 "잠복기(Incubation)"란?',opts:['집중적 몰입 단계','잠시 문제에서 멀어지는 단계','결과 검증 단계','아이디어 발표 단계'],correct:1},
      {q:'두 개념 연결로 새 아이디어를 만드는 Koestler의 기법은?',opts:['브레인스토밍','SCAMPER','비스코포어(Bisociation)','마인드맵'],correct:2},
      {q:'Edward de Bono의 "수평적 사고"란?',opts:['논리적·수직적 추론','비선형 창의적 사고','분석적 비판','귀납적 추론'],correct:1},
      {q:'창의성 저해 요인 중 가장 큰 것은?',opts:['지식 부족','고정관념과 기능적 고착','시간 부족','자원 부족'],correct:1},
      {q:'TRIZ(트리즈)는 어느 나라에서 개발된 창의적 문제 해결 방법론인가?',opts:['미국','독일','러시아','일본'],correct:2},
      {q:'"역브레인스토밍"의 핵심 원리는?',opts:['더 많은 아이디어 생산','문제를 악화시키는 방법 탐색','최선의 아이디어 선택','전문가 의견 수렴'],correct:1},
      {q:'발산적 사고의 측정 지표가 아닌 것은?',opts:['유창성','유연성','독창성','수렴성'],correct:3},
      {q:'창의적 사고와 가장 관련 깊은 뇌파는?',opts:['알파파(α)','베타파(β)','델타파(δ)','세타파(θ)'],correct:3},
      {q:'6가지 색깔 모자 기법(Six Thinking Hats)의 개발자는?',opts:['Tony Buzan','Edward de Bono','Alex Osborn','Roger von Oech'],correct:1}
    ],
    scoreRanges:[
  ...
  cog_flex:{id:'cog_flex',title:'인지 유연성',icon:'🔄',color:'#0f766e',mean:73,sd:9,
    questions:[
      {q:'새로운 규칙에 빠르게 적응하는 인지 능력은?',opts:['집중력','인지 유연성','작업 기억','처리 속도'],correct:1},
      {q:'스트룹(Stroop) 효과란?',opts:['기억 왜곡 현상','색상명과 실제 색 불일치 시 반응이 느려지는 현상','집중력 저하 현상','언어 처리 오류'],correct:1},
      {q:'"기능적 고착(Functional Fixedness)"을 극복하는 방법은?',opts:['기존 기능만 활용','물체를 다른 용도로 상상하기','전문가에게 의뢰','다른 도구 구매'],correct:1},
      {q:'인지 재구성(Cognitive Reframing)이란?',opts:['문제를 회피','문제를 다른 관점으로 재해석','문제를 단순화','전문가에게 맡기기'],correct:1},
      {q:'규칙 변경: 3→9→27에서 2→?→?→16. 두 번째 항은?',opts:['4','6','8','10'],correct:0},
      {q:'인지 유연성이 낮은 사람의 특징은?',opts:['변화에 잘 적응한다','다양한 관점을 수용한다','한 가지 방법을 고집한다','창의적으로 문제를 해결한다'],correct:2},
      {q:'낯선 문화권 사람과 소통 시 인지 유연성이 높은 반응은?',opts:['자신의 방식을 고수한다','상대 문화를 이해하려 노력한다','통역사를 찾는다','의사소통을 포기한다'],correct:1},
      {q:'멀티태스킹이 어려운 인지과학적 이유는?',opts:['뇌 용량이 작아서','주의 자원이 한정되어 있어서','동기 부족','체력 부족'],correct:1},
      {q:'1→3→9에서 규칙을 반전 적용하면 9→?',opts:['3','6','27','81'],correct:0},
      {q:'심리적 유연성을 키우는 가장 효과적인 방법은?',opts:['루틴 고수','다양한 경험과 관점에 노출','집중 훈련','전문가 상담'],correct:1}
    ],
    scoreRanges:[
